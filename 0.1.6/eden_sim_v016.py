...
Notas:
    Esta versión es una base 0.1.6 con interfaz curses estable, comandos persistentes, mapas limpios y estadísticas históricas.
...
class Human:
    ...
    def label(self) -> str:
        ...
        if self.father_label or self.mother_label:
            return f"{self.father_label},{self.mother_label} {self.child_number} {self.person_id}"
    ...
    def try_reproduction(self) -> None:
        ...
        for woman in women:
            ...
            if chance(PROB_REPRODUCCION_DIARIA_BASE):
                ...
                name = f"{father_label},{mother_label} {numero_hijo_madre} {self.person_count}"
...
def spawn_humans_command(world: World, sex_word: str, age_word: str, count_word: str) -> str:
    ...
    for _ in range(count):
        ...
        name = f"spawn,spawn 0 {world.person_count}"
    ...
    return f"Aparecen {count} humano(s), sexo {sex}, edad {age}."
...
def process_command(world: World, command: str) -> str:
    ...
    try:
        if cmd == "help":
            ...
            if len(parts) < 2:
                return "Uso: spawn human hombre 18 3  /  spawn animal conejo 10"
...
def draw_ui(stdscr, world: World, command_buffer: str, last_output: str, color_pairs: Dict[int, int]) -> None:
    ...
    addstr_safe(stdscr, 0, 0, "EDEN SIM 0.1.6 — curses estable, comandos persistentes y mapas limpios", curses.color_pair(4))
    addstr_safe(stdscr, 1, 0, world_status_string(world), curses.color_pair(1))

    living = sorted_living_humans(world)
    if not living:
        addstr_safe(stdscr, 3, 0, "La humanidad se ha extinguido.", curses.color_pair(3))
    else:
        # Calcular radio dinámico para que quepan más mapas sin romperse.
        # Si hay mucha gente, baja un poco el radio.
        n = len(living)
        gap = 2
        min_radius = 5
        radius = VIEW_RADIUS
        ...
        for h in living:
            ...
            draw_colored_window(stdscr, 3, x, window, color_pairs)
            x += window_width + gap
            shown += 1

        if shown < len(living):
            addstr_safe(stdscr, 3 + radius * 2 + 4, 0, f"... {len(living) - shown} mapas más no caben. Amplía terminal o usa Cmd -.", curses.color_pair(6))
...
def main() -> None:
    ...
    if SHOW_START_LEGEND:
        clear_screen()
        print_full_legend()
        print(color("NOVEDAD 0.1.6:", "cyan"))
        print("Interfaz curses estable: los comandos ya no desaparecen al escribir.")
        print("Los mapas no se cortan con los logs ni con los datos.")
        print("Año inicial: 0.")
        print("Los nacidos usan formato: padre,madre número_de_hijo número_de_persona.")
        print("Ejemplos: Adán,Eva 1 3  /  8,5 2 17")
        print()
