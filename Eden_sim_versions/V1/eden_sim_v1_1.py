...
Notas:
    Esta es la versión 1.1 con aprendizaje generacional, estadísticas individuales, civilización progresiva y nuevos comandos.
...
class Human:
    ...
    def label(self) -> str:
        ...
        if self.father_label or self.mother_label:
            ...
            return f"{self.father_label} {self.child_number} {self.person_id}"
    ...
    def try_reproduction(self) -> None:
        ...
        for woman in women:
            ...
            if chance(PROB_REPRODUCCION_DIARIA_BASE):
                ...
                baby.generation = max(father.generation, woman.generation) + 1
...
def spawn_humans_command(world: World, sex_word: str, age_word: str, count_word: str) -> str:
    ...
    for _ in range(count):
        ...
        name = f"spawn 0 {world.person_count}"
...
def human_statistics_text(world: World, target: Optional[str] = None) -> str:
    ...
    if not humans:
        return "No hay humanos vivos para mostrar estadísticas."

    lines = ["ESTADÍSTICAS DE HUMANOS", "-" * 90]
    for h in sorted(humans, key=lambda x: x.person_id or 999999):
        cond = h.condition["name"] if h.condition else "sin condición"
        lines.append(
            f"{h.label()} | sexo={h.sex} | gen={h.generation} | edad={h.age:.1f} | "
...
def is_log_screen(text: str) -> bool:
    return text.startswith("REGISTROS RELEVANTES") or text.startswith("ESTADÍSTICAS DE HUMANOS") or text.startswith("COMANDOS")
...
def draw_log_screen(stdscr, world: World, command_buffer: str, last_output: str) -> None:
    ...
    addstr_safe(stdscr, 0, 0, "EDEN SIM v1.1 — PANEL", curses.color_pair(4))
...
def draw_ui(stdscr, world: World, command_buffer: str, last_output: str, color_pairs: Dict[int, int]) -> None:
    ...
    addstr_safe(stdscr, 0, 0, "EDEN SIM v1.1 — aprendizaje, civilización y estadísticas", curses.color_pair(4))
    addstr_safe(stdscr, 1, 0, world_status_string(world), curses.color_pair(1))
...
def main() -> None:
    ...
    if SHOW_START_LEGEND:
        clear_screen()
        print_full_legend()
        print(color("NOVEDAD v1.1:", "cyan"))
        print("Si spawneas humanos tras una extinción, el tiempo vuelve a avanzar.")
        print("Añadido aprendizaje generacional, herencia de habilidades y defectos, armas, estadísticas y civilización progresiva.")
        print("Año inicial: 0.")
        print("Nuevo comando: spawn humans 18 10 crea humanos equilibrados 50/50. Nuevo comando: estadistica.")
        print("Ejemplos: Adán,Eva 1 3  /  8,5 2 17  /  spawn 0 8")
        print()
