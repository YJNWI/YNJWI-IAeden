...
Notas:
    Esta es la versión 1.3.5: igual que la 1.3.3, pero smap abre un visor gráfico completo 256x256 con colores.
...
class World:
    ...
    def step(self) -> None:
        ...
        # SMAP: solo exporta el mapa si el usuario lo ha abierto.
        write_smap_snapshot(self)
...
def install_runtime_state(world: World) -> None:
    ...
    # SMAP: visor externo del mapa completo. No afecta a la IA.
    world.smap_enabled = False
    world.smap_path = os.path.join(tempfile.gettempdir(), "eden_smap_snapshot.txt")
    world.smap_viewer_path = os.path.join(tempfile.gettempdir(), "eden_smap_terminal_viewer.py")
...
def full_map_letter_lines(world: World) -> List[str]:
    """
    Mapa completo 256x256 en letras.
    Solo lectura. No cambia IA, movimiento ni supervivencia.
    """
...
def write_smap_snapshot(world: World, force: bool = False) -> None:
    """
    Escribe un TXT con el mapa completo. Lo lee una segunda ventana.
    """
    if not getattr(world, "smap_enabled", False):
        return

    now = time.time()
    if not force and now - getattr(world, "smap_last_write", 0.0) < 0.50:
        return
    ...
    body = full_map_letter_lines(world)

    tmp_path = world.smap_path + ".tmp"
    with open(tmp_path, "w", encoding="utf-8") as f:
        f.write("\\n".join(header + body))
        f.write("\\n")
    os.replace(tmp_path, world.smap_path)


def write_smap_terminal_viewer(path: str) -> None:
    """
    v1.3.5:
    Aunque conserva el nombre interno para no tocar más código,
    ahora escribe un visor gráfico Tkinter, no un visor de Terminal.
    """
    viewer = '#!/usr/bin/env python3\n# -*- coding: utf-8 -*-\nimport os\nimport sys\nimport time\nimport tkinter as tk\nfrom tkinter import ttk\n\nsnapshot = sys.argv[1] if len(sys.argv) > 1 else ""\n\nCELL = 4\nGRID_W = 256\nGRID_H = 256\n\nCOLORS = {\n    # Terreno\n    ".": "#355e3b",  # pradera\n    "T": "#0b7a2a",  # bosque\n    "~": "#2277dd",  # agua\n    "C": "#c9c2a3",  # cueva\n    "M": "#777777",  # montaña\n    "S": "#37685c",  # pantano\n    "h": "#9b59b6",  # choza\n    "H": "#8e44ad",  # casa\n    "B": "#5b2c6f",  # edificio\n    "R": "#8b5a2b",  # casa/refugio marrón\n\n    # Humanos\n    "A": "#00ffff",\n    "E": "#ff66ff",\n    "@": "#39d5ff",\n\n    # Plantas/comida\n    "f": "#7CFC00",\n    "b": "#b7ff3c",\n    "n": "#b7ff3c",\n    "m": "#b7ff3c",\n    "a": "#b7ff3c",\n\n    # Animales pacíficos\n    "c": "#f4d03f",\n    "d": "#f5b041",\n    "p": "#85c1e9",\n    "g": "#f7dc6f",\n    "k": "#f8c471",\n\n    # Depredadores\n    "L": "#ff3333",\n    "O": "#ff3333",\n    "V": "#ff3333",\n    "J": "#ff3333",\n    "P": "#ff3333",\n    "X": "#ff0000",\n}\nDEFAULT = "#dddddd"\n\nroot = tk.Tk()\nroot.title("EDEN SIM — SMAP gráfico 256x256")\nroot.geometry("1250x900")\nroot.configure(bg="#111827")\n\ntop = ttk.Frame(root)\ntop.pack(side="top", fill="x")\n\nstatus_var = tk.StringVar(value="Esperando snapshot...")\nttk.Label(top, textvariable=status_var).pack(side="left", padx=8, pady=4)\n\nzoom_var = tk.IntVar(value=CELL)\n\ndef set_zoom(delta):\n    global CELL\n    CELL = max(2, min(12, CELL + delta))\n    zoom_var.set(CELL)\n    redraw(force=True)\n\nttk.Button(top, text="- zoom", command=lambda: set_zoom(-1)).pack(side="right", padx=4)\nttk.Button(top, text="+ zoom", command=lambda: set_zoom(1)).pack(side="right", padx=4)\nttk.Label(top, text="Zoom").pack(side="right", padx=4)\n\nlegend = ttk.Label(\n    root,\n    text="A/E/@ humanos | ~=río | T=bosque | C=cueva | M=montaña | S=pantano | R=refugio | X=T-Rex",\n)\nlegend.pack(side="bottom", fill="x")\n\nframe = ttk.Frame(root)\nframe.pack(fill="both", expand=True)\n\ncanvas = tk.Canvas(frame, bg="#0b1020", highlightthickness=0)\nys = ttk.Scrollbar(frame, orient="vertical", command=canvas.yview)\nxs = ttk.Scrollbar(frame, orient="horizontal", command=canvas.xview)\ncanvas.configure(yscrollcommand=ys.set, xscrollcommand=xs.set)\n\ncanvas.grid(row=0, column=0, sticky="nsew")\nys.grid(row=0, column=1, sticky="ns")\nxs.grid(row=1, column=0, sticky="ew")\nframe.rowconfigure(0, weight=1)\nframe.columnconfigure(0, weight=1)\n\nlast_content = ""\nlast_grid = []\nlast_status = ""\n\ndef parse_snapshot(content):\n    lines = content.splitlines()\n    status = ""\n    grid = []\n\n    if len(lines) >= 2:\n        status = lines[1]\n\n    # Busca la línea separadora de 256 guiones.\n    start = None\n    for i, line in enumerate(lines):\n        if len(line) >= 200 and set(line.strip()) == {"-"}:\n            start = i + 1\n            break\n\n    if start is not None:\n        grid = lines[start:start + GRID_H]\n\n    return status, grid\n\ndef redraw(force=False):\n    global last_grid, last_status\n\n    if not last_grid:\n        return\n\n    y0 = canvas.yview()[0] if canvas.yview() else 0\n    x0 = canvas.xview()[0] if canvas.xview() else 0\n\n    canvas.delete("all")\n\n    cell = CELL\n    w = GRID_W * cell\n    h = GRID_H * cell\n    canvas.configure(scrollregion=(0, 0, w, h))\n\n    # Dibujo con cuadrados. Para cell >= 6 también escribe letras en entidades importantes.\n    for y, row in enumerate(last_grid[:GRID_H]):\n        for x, ch in enumerate(row[:GRID_W]):\n            color = COLORS.get(ch, DEFAULT)\n            x1 = x * cell\n            y1 = y * cell\n            canvas.create_rectangle(x1, y1, x1 + cell, y1 + cell, fill=color, outline=color)\n            if cell >= 6 and ch in "AE@XLR":\n                canvas.create_text(\n                    x1 + cell / 2,\n                    y1 + cell / 2,\n                    text=ch,\n                    fill="#000000" if ch in "AE@" else "#ffffff",\n                    font=("Menlo", max(5, cell - 1), "bold"),\n                )\n\n    status_var.set(last_status or "SMAP activo")\n    canvas.xview_moveto(x0)\n    canvas.yview_moveto(y0)\n\ndef refresh():\n    global last_content, last_grid, last_status\n    try:\n        if snapshot and os.path.exists(snapshot):\n            with open(snapshot, "r", encoding="utf-8") as f:\n                content = f.read()\n        else:\n            content = ""\n\n        if content and content != last_content:\n            last_content = content\n            status, grid = parse_snapshot(content)\n            if grid:\n                last_status = status\n                last_grid = grid\n                redraw(force=True)\n            else:\n                status_var.set("Snapshot leído, pero no encuentro mapa 256x256.")\n\n        elif not content:\n            status_var.set("Esperando snapshot del simulador...")\n\n    except Exception as exc:\n        status_var.set(f"Error leyendo snapshot: {exc}")\n\n    root.after(700, refresh)\n\n# Movimiento cómodo con rueda de ratón.\ndef on_mousewheel(event):\n    if event.state & 0x0001:  # Shift = horizontal\n        canvas.xview_scroll(int(-1 * (event.delta / 120)), "units")\n    else:\n        canvas.yview_scroll(int(-1 * (event.delta / 120)), "units")\n\ncanvas.bind_all("<MouseWheel>", on_mousewheel)\n\nrefresh()\nroot.mainloop()\n'
    with open(path, "w", encoding="utf-8") as f:
        f.write(viewer)


def open_smap_window(world: World) -> str:
    """
    Abre una ventana gráfica Python/Tkinter con el mapa completo.
    No modifica IA, movimiento ni supervivencia.
    """
    proc = getattr(world, "smap_process", None)
    if proc is not None and proc.poll() is None:
        world.smap_enabled = True
        write_smap_snapshot(world, force=True)
        return "SMAP gráfico ya estaba abierto y se sigue actualizando."

    world.smap_enabled = True
    write_smap_snapshot(world, force=True)
    write_smap_terminal_viewer(world.smap_viewer_path)

    try:
        world.smap_process = subprocess.Popen(
            [sys.executable, world.smap_viewer_path, world.smap_path],
            stdout=subprocess.DEVNULL,
            stderr=subprocess.DEVNULL,
        )
        world.log("Comando: smap gráfico abierto en una ventana externa.")
        return "SMAP gráfico abierto: mapa completo 256x256 con colores, zoom y scroll."
    except Exception as exc:
        world.smap_enabled = False
        return f"No se pudo abrir SMAP gráfico: {exc}"
