...
Notas:
    Esta versión es una base 0.1.3b con ventana gráfica robusta aunque pygame.font no esté disponible.
...
# ============================================================
# CONSOLA, COMANDOS Y VENTANA GRÁFICA
# ============================================================
...
PLANT_GOOD_RGB = (40, 230, 85)
PLANT_BAD_RGB = (230, 35, 65)
ANIMAL_PEACE_RGB = (240, 210, 70)
ANIMAL_AGGR_RGB = (220, 45, 45)
HUMAN_MALE_RGB = (50, 220, 255)
HUMAN_FEMALE_RGB = (255, 75, 210)
HUMAN_OTHER_RGB = (255, 255, 255)
...
def install_runtime_state(world: World) -> None:
    ...
    world.paused = False
    world.speed_steps = 1
    world.step_delay = 0.08
    world.running = True
    world.relevant_events = []
    world.last_terminal_status = 0.0
...
def world_status_string(world: World) -> str:
    ...
    return (
        ...
        f"Humanos: {len(living)} | Pacíficos: {peaceful} | Depredadores: {aggressive} | "
        f"Plantas: {len(world.plants)} | Edificios: {world.buildings} | "
        f"Conocimiento: {', '.join(techs) if techs else 'ninguno'}"
    )


def console_print_status(world: World) -> None:
    print(world_status_string(world), flush=True)
...
def patched_log(self: World, msg: str) -> None:
    ...
    print(f"\n[EVENTO] {line}", flush=True)


World.log = patched_log


def print_relevant_logs(world: World, amount: Optional[int] = None) -> None:
    logs = getattr(world, "relevant_events", [])
    if not logs:
        print("No hay registros relevantes todavía.")
        return

    selected = logs if amount is None else logs[-amount:]

    print()
    print(color("REGISTROS RELEVANTES", "yellow"))
    print("-" * 70)
    for line in selected:
        print(line)
    print("-" * 70)
    console_print_status(world)
...
def find_human_by_name(world: World, name: str) -> Optional[Human]:
    target = name.lower()
...
def spawn_humans_command(world: World, sex_word: str, age_word: str, count_word: str) -> None:
    ...
    try:
        ...
        print("Uso: spawn human hombre 18 3")
        return
    ...
    for _ in range(count):
        world.birth_count += 1
        name = f"Spawn_{world.birth_count}"
...
def spawn_animals_command(world: World, animal_id: str, count_word: str) -> None:
    try:
        count = int(count_word)
    except ValueError:
        print("Uso: spawn animal conejo 10")
        return
    ...
    if animal_id not in valid:
        print("Animal no válido. Usa uno de estos:")
        print(", ".join(valid))
        return
...
def heal_command(world: World, target: str, amount_word: str) -> None:
    try:
        amount = float(amount_word)
    except ValueError:
        print("Uso: heal all 30  /  heal Adán 30")
        return
    ...
    if not humans:
        print("No he encontrado humanos para curar.")
        return
...
def setlife_command(world: World, target: str, value_word: str) -> None:
    try:
        value = float(value_word)
    except ValueError:
        print("Uso: setlife all 100  /  setlife Eva 100")
        return
    ...
    if not humans:
        print("No he encontrado humanos.")
        return
...
def resource_command(world: World, stat: str, target: str, value_word: str) -> None:
    try:
        value = float(value_word)
    except ValueError:
        print(f"Uso: {stat} all 100")
        return
...
def give_command(world: World, target: str, item: str, amount_word: str) -> None:
    try:
        amount = int(amount_word)
    except ValueError:
        print("Uso: give all food 10  /  give Eva wood 20")
        return

    valid_items = ["food", "raw_meat", "wood", "stone"]

    if item not in valid_items:
        print("Objeto no válido:", ", ".join(valid_items))
        return
...
def set_weather_command(world: World, weather_name: str) -> None:
    valid = [
...
def disaster_command(world: World, disaster_name: str) -> None:
    if disaster_name == "incendio":
        world.forest_fire()
        return

    if disaster_name not in ["tornado", "terremoto", "inundacion"]:
        print("Desastres: tornado, terremoto, inundacion, incendio")
        return
...
def print_help() -> None:
    print()
    print(color("COMANDOS DISPONIBLES", "cyan"))
    print("  help                         Muestra esta ayuda")
    print("  status                       Muestra estado actual")
    print("  logs                         Muestra todos los eventos relevantes")
    print("  logs 30                      Muestra los últimos 30 eventos relevantes")
    print("  pause                        Pausa la simulación")
    print("  resume                       Reanuda la simulación")
    print("  speed 1                      Velocidad normal")
    print("  speed 5                      Simula 5 horas por ciclo")
    print("  delay 0.08                   Segundos reales entre ciclos")
    print()
    print("  spawn human hombre 18 3      Crea 3 hombres de 18 años")
    print("  spawn human mujer 18 3       Crea 3 mujeres de 18 años")
    print("  spawn animal conejo 20       Crea 20 conejos")
    print("  spawn animal lobo 2          Crea 2 lobos")
    print()
    print("  heal all 30                  Suma 30 de vida a todos")
    print("  heal Eva 30                  Suma 30 de vida a Eva")
    print("  setlife all 100              Fija vida de todos a 100")
    print("  food all 100                 Fija hambre/saciedad a 100")
    print("  water all 100                Fija sed/hidratación a 100")
    print("  energy all 100               Fija energía a 100")
    print("  give all food 10             Da comida al inventario")
    print("  give all wood 30             Da madera")
    print()
    print("  weather lluvia               Cambia el clima")
    print("  weather tormenta             Cambia el clima a tormenta")
    print("  disaster terremoto           Provoca terremoto")
    print("  disaster incendio            Provoca incendio forestal")
    print()
    print("Controles en ventana:")
...
def process_command(world: World, command: str) -> None:
    ...
    try:
        if cmd == "help":
            ...
            if len(parts) >= 2:
                amount = int(parts[1])
            print_relevant_logs(world, amount)
            ...
            if len(parts) < 2:
                print(f"Velocidad actual: {world.speed_steps}")
            else:
                world.speed_steps = max(1, min(int(parts[1]), 200))
                print(f"Velocidad: {world.speed_steps} hora(s) simuladas por ciclo.")

        elif cmd == "delay":
            if len(parts) < 2:
                print(f"Delay actual: {world.step_delay}")
            else:
                world.step_delay = max(0.0, min(float(parts[1]), 3.0))
                print(f"Delay: {world.step_delay} segundos.")

        elif cmd == "spawn":
            if len(parts) < 2:
                print("Uso: spawn human hombre 18 3  /  spawn animal conejo 10")
            elif parts[1].lower() == "human" and len(parts) >= 5:
                spawn_humans_command(world, parts[2], parts[3], parts[4])
            elif parts[1].lower() == "animal" and len(parts) >= 4:
                spawn_animals_command(world, parts[2], parts[3])
            else:
                print("Uso: spawn human hombre 18 3  /  spawn animal conejo 10")

        elif cmd == "heal" and len(parts) >= 3:
            heal_command(world, parts[1], parts[2])

        elif cmd == "setlife" and len(parts) >= 3:
            setlife_command(world, parts[1], parts[2])

        elif cmd in ["food", "water", "energy"] and len(parts) >= 3:
            resource_command(world, cmd, parts[1], parts[2])

        elif cmd == "give" and len(parts) >= 4:
            give_command(world, parts[1], parts[2], parts[3])

        elif cmd == "weather" and len(parts) >= 2:
            set_weather_command(world, parts[1])

        elif cmd == "disaster" and len(parts) >= 2:
            disaster_command(world, parts[1])
...
def command_input_loop(world: World) -> None:
    print()
    print(color("Consola lista. Escribe help para ver comandos.", "cyan"))

    while getattr(world, "running", True):
        try:
            command = input("> ")
            world.command_queue.put(command)
        except EOFError:
            break
        except KeyboardInterrupt:
            world.command_queue.put("pause")
            break


def draw_text(screen, font, text, x, y, color_rgb=(230, 230, 230)) -> int:
    """
    Dibuja texto si pygame.font está disponible.
    En algunos Python muy nuevos, como 3.14, pygame puede instalarse sin pygame.font.
    En ese caso no rompemos la ventana: el texto del estado sigue saliendo por terminal.
    """
    if font is None:
        return y + 16

    surf = font.render(text, True, color_rgb)
    screen.blit(surf, (x, y))
    return y + surf.get_height() + 4


def draw_gui(world: World, pygame, screen, font, small_font) -> None:
    screen.fill((10, 10, 14))

    # Mapa base.
    for y in range(MAP_H):
        for x in range(MAP_W):
            rgb = TILE_RGB.get(world.map[y][x], (60, 60, 60))
            pygame.draw.rect(
                screen,
                rgb,
                (x * GUI_CELL_SIZE, y * GUI_CELL_SIZE, GUI_CELL_SIZE, GUI_CELL_SIZE),
            )

    # Plantas.
    for p in world.plants:
        rgb = PLANT_BAD_RGB if p.data["poison"] else PLANT_GOOD_RGB
        pygame.draw.rect(
            screen,
            rgb,
            (p.x * GUI_CELL_SIZE, p.y * GUI_CELL_SIZE, GUI_CELL_SIZE, GUI_CELL_SIZE),
        )
    ...
    for a in world.animals:
        if not a.alive:
            continue
        rgb = ANIMAL_AGGR_RGB if a.aggressive else ANIMAL_PEACE_RGB
        pygame.draw.rect(
            screen,
            rgb,
            (a.x * GUI_CELL_SIZE, a.y * GUI_CELL_SIZE, GUI_CELL_SIZE, GUI_CELL_SIZE),
        )

    # Humanos encima de todo.
    for h in world.living_humans():
        if h.name == "Adán":
            rgb = (255, 255, 255)
        elif h.name == "Eva":
            rgb = HUMAN_FEMALE_RGB
        elif h.sex == "hombre":
            rgb = HUMAN_MALE_RGB
        else:
            rgb = HUMAN_FEMALE_RGB
        ...
        pygame.draw.rect(
            screen,
            rgb,
            (h.x * GUI_CELL_SIZE, h.y * GUI_CELL_SIZE, GUI_CELL_SIZE + 2, GUI_CELL_SIZE + 2),
        )

    # Panel lateral.
    panel_x = MAP_W * GUI_CELL_SIZE + 12
    y = 12

    y = draw_text(screen, font, "EDEN SIM 0.1.3b", panel_x, y, (80, 220, 255))
    y = draw_text(screen, small_font, world_time_string(world), panel_x, y)
    y += 8

    living = world.living_humans()
    peaceful = sum(1 for a in world.animals if not a.aggressive)
    aggressive = sum(1 for a in world.animals if a.aggressive)
    y = draw_text(screen, small_font, f"Humanos vivos: {len(living)}", panel_x, y)
    y = draw_text(screen, small_font, f"Animales pacíficos: {peaceful}", panel_x, y)
    y = draw_text(screen, small_font, f"Depredadores: {aggressive}", panel_x, y)
    y = draw_text(screen, small_font, f"Plantas: {len(world.plants)}", panel_x, y)
    y = draw_text(screen, small_font, f"Edificios: {world.buildings}", panel_x, y)
    y = draw_text(screen, small_font, f"Velocidad: x{world.speed_steps}", panel_x, y)
    y = draw_text(screen, small_font, f"Estado: {'PAUSA' if world.paused else 'RUN'}", panel_x, y)
    y += 8

    techs = [k for k, v in world.tribe_knowledge.items() if v]
    y = draw_text(screen, small_font, "Conocimiento:", panel_x, y, (255, 230, 120))
    if techs:
        for t in techs[:8]:
            y = draw_text(screen, small_font, f"- {t}", panel_x + 8, y)
    else:
        y = draw_text(screen, small_font, "- ninguno", panel_x + 8, y)

    y += 8
    y = draw_text(screen, small_font, "Top humanos:", panel_x, y, (255, 230, 120))
    for h in sorted(living, key=lambda x: x.fitness, reverse=True)[:8]:
        y = draw_text(
            screen,
            small_font,
            f"{h.name} {h.sex[:1]} V{h.life:.0f} H{h.hunger:.0f} S{h.thirst:.0f} E{h.energy:.0f}",
            panel_x + 8,
            y,
        )

    y += 8
    y = draw_text(screen, small_font, "Controles:", panel_x, y, (255, 230, 120))
    for line in ["ESPACIO pausa", "+ acelerar", "- ralentizar", "ESC salir", "Terminal: help"]:
        y = draw_text(screen, small_font, line, panel_x + 8, y)

    # Barra inferior visual de tiempo.
    bottom_y = MAP_H * GUI_CELL_SIZE + 8
    pygame.draw.rect(screen, (22, 22, 30), (0, MAP_H * GUI_CELL_SIZE, MAP_W * GUI_CELL_SIZE + GUI_PANEL_W, 34))
    draw_text(screen, small_font, world_status_string(world), 8, bottom_y, (220, 220, 220))


def run_pygame_simulation(world: World) -> None:
    try:
        import pygame
    except ImportError:
        print(color("No está instalado pygame.", "red"))
        print("Instálalo con:")
        print("  pip install pygame")
        print()
        print("Mientras tanto puedes seguir usando la versión de terminal.")
        return

    pygame.init()
    pygame.display.set_caption("Eden Sim 0.1.3b")

    width = MAP_W * GUI_CELL_SIZE + GUI_PANEL_W
    height = MAP_H * GUI_CELL_SIZE + 34

    screen = pygame.display.set_mode((width, height))

    # pygame.font puede no estar disponible en algunas instalaciones de Python 3.14.
    # Si falla, la ventana gráfica seguirá funcionando y el texto saldrá por terminal.
    font = None
    small_font = None
    try:
        ...
        print()
        print(color("Aviso: pygame.font no está disponible en tu instalación.", "yellow"))
        print("La ventana se abrirá sin textos en el panel lateral.")
        print("Los eventos, comandos y estado seguirán saliendo por terminal.")
        print(f"Detalle técnico: {e}")
        print()

    clock = pygame.time.Clock()

    input_thread = threading.Thread(target=command_input_loop, args=(world,), daemon=True)
    input_thread.start()
    ...
    while world.running:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_ESCAPE:
                    ...
                    print("Pausado." if world.paused else "Reanudado.")
                elif event.key in [pygame.K_PLUS, pygame.K_EQUALS, pygame.K_KP_PLUS]:
                    world.speed_steps = min(world.speed_steps + 1, 200)
                    print(f"Velocidad: x{world.speed_steps}")
                elif event.key in [pygame.K_MINUS, pygame.K_KP_MINUS]:
                    world.speed_steps = max(world.speed_steps - 1, 1)
                    print(f"Velocidad: x{world.speed_steps}")

        while not world.command_queue.empty():
            process_command(world, world.command_queue.get())

        now = time.time()

        if not world.paused and now - last_step >= world.step_delay:
            for _ in range(world.speed_steps):
                if not world.living_humans() and not world.extinction_report_printed:
                    world.log("La humanidad se ha extinguido.")
                    print_relevant_logs(world, amount=None)
        ...
        # Estado temporal en terminal cada 4 segundos reales.
        # En la ventana gráfica también aparece siempre en la barra inferior.
        if now - last_status >= 4:
            console_print_status(world)
            last_status = now

        draw_gui(world, pygame, screen, font, small_font)
        pygame.display.flip()
        clock.tick(GUI_FPS)

    pygame.quit()
    print()
    print(color("Simulación cerrada.", "yellow"))
    print_relevant_logs(world, amount=50)
...
def main() -> None:
    ...
    if SHOW_START_LEGEND:
        clear_screen()
        print_full_legend()
        print(color("NOVEDAD 0.1.3:", "cyan"))
        print("Se abrirá una ventana gráfica para el mapa.")
        print("La terminal queda para eventos en tiempo real y comandos.")
        print("Escribe help cuando empiece para ver comandos.")
        print()
        if WAIT_AFTER_START_LEGEND:
            input("Pulsa ENTER para crear el mundo y abrir la ventana...")

    world = World()
    install_runtime_state(world)
    world.generate()

    run_pygame_simulation(world)
