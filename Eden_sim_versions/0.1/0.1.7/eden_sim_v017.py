...
Notas:
    Esta versión es una base 0.1.7 con reactivación tras extinción y mapas en parrilla de 6 por fila.
...
def draw_ui(stdscr, world: World, command_buffer: str, last_output: str, color_pairs: Dict[int, int]) -> None:
    ...
    addstr_safe(stdscr, 0, 0, "EDEN SIM 0.1.7 — mapas en parrilla, reactivación tras extinción", curses.color_pair(4))
    addstr_safe(stdscr, 1, 0, world_status_string(world), curses.color_pair(1))

    living = sorted_living_humans(world)

    # Panel inferior fijo. Lo reservamos antes de dibujar mapas.
    panel_h = 7
    ...
    if not living:
        ...
        # Queremos 6 mapas por fila siempre que quepan.
        # Si la terminal no da anchura, bajamos radio para intentar mantener 6.
        maps_per_row = 6
        gap = 2
        min_radius = 3
        radius = VIEW_RADIUS
        ...
        for idx, h in enumerate(living):
            ...
            y0 = start_y + row * (window_height + 1)
            x0 = col * (window_width + gap)

            if y0 + window_height >= panel_y:
                hidden_count += 1
                continue
        ...
        if hidden_count > 0:
            addstr_safe(
                ...
                max(3, panel_y - 1),
                0,
                f"... {hidden_count} mapa(s) no caben en pantalla. Amplía terminal, usa Cmd -, o baja VIEW_RADIUS.",
                curses.color_pair(6),
            )
...
def main() -> None:
    ...
    if SHOW_START_LEGEND:
        clear_screen()
        print_full_legend()
        print(color("NOVEDAD 0.1.7:", "cyan"))
        print("Si spawneas humanos tras una extinción, el tiempo vuelve a avanzar.")
        print("Los mapas aparecen en parrilla: 6 por fila, luego segunda fila, tercera, etc.")
        print("Año inicial: 0.")
