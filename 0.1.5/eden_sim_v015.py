...
Notas:
    Esta versión es una base 0.1.5 con terminal estable sin parpadeo, mapas dinámicos por humano vivo y accidentes revisados.
...
class Human:
    ...
    def decide_action(self, world: "World") -> str:
        ...
        # Control anti-obsesión por construir:
        # construir solo tiene sentido si no hay ya demasiados refugios cerca,
        # si tiene energía suficiente y si la situación básica está estable.
        if action == "construir":
            nearby_huts = world.count_tiles_close(self.x, self.y, HUT, 8)
            if nearby_huts >= 2 or self.energy < 55 or self.hunger < 55 or self.thirst < 55:
    ...
    def build(self, world: "World") -> None:
        # Construcción inicial controlada.
        # En versiones anteriores podía obsesionarse con construir chozas sin parar.
        # Ahora solo se construye si hay necesidad real y límites cercanos.
        tile = world.map[self.y][self.x]
        ...
        # Recoger madera si hay bosque cerca.
        if world.tile_close(self.x, self.y, FOREST, 4):
            self.inventory["wood"] += 1
            self.energy -= 1.4

        # Coste más alto para evitar spam de chozas.
        hut_cost = 14

        # Si ya hay cueva/refugio cerca, la prioridad de construir baja mucho.
        if nearby_shelters >= 2 and chance(0.80):
            self.explore(world)
            return

        if self.inventory["wood"] >= hut_cost and tile in [GRASS, FOREST]:
    ...
    def try_reproduction(self) -> None:
        ...
        for woman in women:
            ...
            if chance(PROB_REPRODUCCION_DIARIA_BASE):
                ...
                name = f"@{father.name},{woman.name}_hijo{numero_hijo_madre}_nombre{self.person_count}"
...
# ============================================================
# CONSOLA, COMANDOS Y MAPA EN TERMINAL
# ============================================================

TERMINAL_RENDER_EVERY_HOURS = 1
COMMAND_PROMPT = "cmd> "
USE_STABLE_TERMINAL_RENDER = True
...
def install_runtime_state(world: World) -> None:
    ...
    world._terminal_initialized = False
    world._last_render_line_count = 0
...
def print_relevant_logs(world: World, amount: Optional[int] = None) -> None:
    ...
    for line in selected:
        print(line)
    print("-" * 90)
    print(world_status_string(world))
...
def spawn_humans_command(world: World, sex_word: str, age_word: str, count_word: str) -> None:
    ...
    for _ in range(count):
        ...
        name = f"@spawn,spawn_hijo0_nombre{world.person_count}"
...
def print_help() -> None:
    ...
    print("  delay 0.20                   Segundos reales entre ciclos")
    print("  render                       Redibuja el mapa")
    print()
...
def non_blocking_read_command() -> Optional[str]:
    """
    Lee comandos sin bloquear la simulación.
    En macOS/Linux funciona con select sobre stdin.
    """
    try:
...
def entity_symbol_at(world: World, x: int, y: int, preferred: Optional[Human] = None) -> str:
    ...
    for a in world.animals:
        if a.alive and a.x == x and a.y == y:
            return color(a.data.get("symbol", "?"), "red" if a.aggressive else "yellow")

    for p in world.plants:
        if p.x == x and p.y == y:
            return color(p.data.get("symbol", "?"), "red" if p.data["poison"] else "lightgreen")

    return terrain_symbol(world.map[y][x])


def render_window_lines(world: World, center_x: int, center_y: int, radius: int, title: str) -> List[str]:
    width = radius * 2 + 1
    title_plain = f" {title} "
    border = "+" + "-" * width + "+"

    lines = [border]
    header = title_plain[:width].center(width)
    lines.append("|" + header + "|")
    ...
    for y in range(min_y, max_y + 1):
        ...
        for x in range(min_x, max_x + 1):
            if not world.in_bounds(x, y):
                ...
                row += entity_symbol_at(world, x, y)
        lines.append("|" + row + "|")
...
def render_dual_maps(world: World) -> List[str]:
    living = world.living_humans()

    if not living:
        return [color("La humanidad se ha extinguido.", "red")]

    def human_sort_key(h: Human):
        if h.name == "Adán":
            return (0, h.name)
        if h.name == "Eva":
            return (1, h.name)
    ...
    humans = sorted(living, key=human_sort_key)

    windows = [
        render_window_lines(world, h.x, h.y, VIEW_RADIUS, f"Mapa de {h.name}")
        for h in humans
    ]
    ...
    # Si hay muchos humanos, la línea puede ser ancha.
    # Lo dejamos así porque queremos que cada nuevo humano aparezca a la derecha del último.
    for i in range(max_len):
        parts = []
        for w in windows:
            parts.append(w[i] if i < len(w) else " " * (VIEW_RADIUS * 2 + 3))
        combined.append(gap.join(parts))

    return combined

def render_terminal(world: World) -> None:
    lines = []

    lines.append(color("EDEN SIM 0.1.5 — terminal estable, mapas dinámicos y accidentes revisados", "cyan"))
    lines.append(world_status_string(world))
    lines.append("")

    lines.extend(render_dual_maps(world))

    lines.append("")
    lines.append(color("Humanos destacados:", "yellow"))
    living = world.living_humans()
    if not living:
        ...
        for h in sorted(living, key=lambda x: x.fitness, reverse=True)[:8]:
            cond = h.condition["name"] if h.condition else "sin condición"
            lines.append(
                f"  {h.name:24s} {h.sex:6s} edad {h.age:5.1f} | "
                f"vida {h.life:5.1f} hambre {h.hunger:5.1f} sed {h.thirst:5.1f} energía {h.energy:5.1f} | "
                f"hijos {h.children} | inv(food={h.inventory.get('food',0)}, wood={h.inventory.get('wood',0)}) | {cond}"
            )

    lines.append("")
    lines.append(color("Eventos recientes:", "yellow"))
    for e in world.relevant_events[-10:]:
        lines.append(" - " + e)

    lines.append("")
    lines.append(color("Comandos: help | status | logs 30 | pause | resume | speed 5 | spawn human hombre 18 3 | quit", "cyan"))
    lines.append(color("Escribe comando y pulsa ENTER. El mapa se actualiza sin limpiar toda la pantalla para evitar parpadeo.", "gray"))

    output = "\n".join(lines)

    if not getattr(world, "_terminal_initialized", False):
        clear_screen()
        sys.stdout.write("\033[?25l")  # ocultar cursor
        world._terminal_initialized = True
    else:
        sys.stdout.write("\033[H")  # cursor al inicio

    sys.stdout.write(output)

    previous = getattr(world, "_last_render_line_count", 0)
    ...
    world._last_render_line_count = len(lines)

def simulation_loop(world: World) -> None:
    print_help()
    render_terminal(world)

    while world.running:
        cmd = non_blocking_read_command()
        if cmd:
        ...
        if not world.paused:
            for _ in range(world.speed_steps):
                ...
                if world.hour != old_hour and world.hour % TERMINAL_RENDER_EVERY_HOURS == 0:
                    render_terminal(world)

        time.sleep(world.step_delay)
...
def main() -> None:
    ...
    if SHOW_START_LEGEND:
        clear_screen()
        print_full_legend()
        print(color("NOVEDAD 0.1.5:", "cyan"))
        print("Terminal estable: mapa con letras y colores sin parpadeo fuerte.")
        print("Ahora hay un mapa por cada humano vivo, de izquierda a derecha.")
        print("Los nacidos aparecen con su propio mapa a la derecha; los muertos desaparecen del render.")
        print()
