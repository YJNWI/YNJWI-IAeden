...
Notas:
    Esta versión es una base 0.1.4 con terminal avanzado, mapas de Adán/Eva lado a lado, comandos y registros.
...
class Human:
    ...
    def build(self, world: "World") -> None:
        ...
        # En versiones anteriores Eva podía obsesionarse con construir chozas sin parar.
...
def render_dual_maps(world: World) -> List[str]:
    ...
    adam = find_human_by_name(world, "Adán")
    eve = find_human_by_name(world, "Eva")

    if not adam or not adam.alive:
        adam = living[0] if living else None

    if not eve or not eve.alive:
        eve = living[1] if len(living) > 1 else adam

    if not living:
        return [color("La humanidad se ha extinguido.", "red")]

    left = render_window_lines(world, adam.x, adam.y, VIEW_RADIUS, f"Mapa de {adam.name}")
    right = render_window_lines(world, eve.x, eve.y, VIEW_RADIUS, f"Mapa de {eve.name}") if eve else []

    combined = []
    max_len = max(len(left), len(right)) if right else len(left)

    for i in range(max_len):
        l = left[i] if i < len(left) else " " * (VIEW_RADIUS * 2 + 3)
...
def render_terminal(world: World) -> None:
    ...
    print(color("EDEN SIM 0.1.4 — terminal avanzado con comandos", "cyan"))
    print(world_status_string(world))
    print()

    for line in render_dual_maps(world):
        print(line)
    ...
    print()
    print(color("Comandos: help | status | logs 30 | pause | resume | speed 5 | spawn human hombre 18 3 | quit", "cyan"))
    print(color("Escribe comando y pulsa ENTER. El mapa se actualiza cada hora simulada.", "gray"))
...
def main() -> None:
    ...
    if SHOW_START_LEGEND:
        clear_screen()
        print_full_legend()
        print(color("NOVEDAD 0.1.4:", "cyan"))
        print("Volvemos al mapa en terminal con letras y colores.")
        print("Ahora hay dos mapas lado a lado: uno centrado en Adán y otro en Eva.")
        print("La terminal acepta comandos sin parar la simulación.")
        print()
